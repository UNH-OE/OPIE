...
%x(even)=y-direction
	...
	if k==1
      ...
      xref=B(1,:);
      d=setAOI2D(A,B(1,:));
  	   xareaC=getAOI2D(A,d,[B(1,1),B(1,2)]);		%Extracts the area of intrest from the image
	   xareaD=getAOI2D(A,d,[B(1,3),B(1,4)]);		%Extracts the area of intrest from the image
	else
...
%Finds points near predicted value
	xareaA=getAOI2D(A,d,[B(k-1,1),B(k-1,2)]);		%Extracts the area of intrest from the image
	xareaB=getAOI2D(A,d,[B(k-1,3),B(k-1,4)]);		%Extracts the area of intrest from the image
	xareaC=makesame(xareaA,xareaC);		%Makes them the same size by padding the new argument with zeros
	xareaD=makesame(xareaB,xareaD);		%Makes them the same size by padding the new argument with zeros
	xc1=fftshift(ifft2(conj(fft2(xareaC)).*fft2(xareaA)));
	xc2=fftshift(ifft2(conj(fft2(xareaD)).*fft2(xareaB)));
	[x1,y1]=getdot2d(xc1);
	[x2,y2]=getdot2d(xc2);
	B(k,1)=B(k-1,1)+x1;
	B(k,2)=B(k-1,2)+y1;
...
%Shows location of point
   ...
   C=drawbox(C,d,B(k,1),B(k,2));
	C=drawbox(C,d,B(k,3),B(k,4));
   imshow(C);
	drawnow;
